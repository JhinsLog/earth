...
/**
 * 내 위치 표시가 나타나기 시작하는 줌.
 *
 * 지구본이 보이는 축척에서는 내 위치 점이 별과 구분되지 않고, 같은 지역에 별이 있으면
 * 어느 쪽이 무엇인지 알 수 없다. 그래서 확대해서 볼 때만 그린다. 판단 기준은 줌 하나뿐이라
 * "내 위치" 버튼으로 이동하든 마우스 휠로 확대하든 동일하게 동작한다.
 *
